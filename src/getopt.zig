// https://github.com/dmgk/zig-getopt OBSD
...
pub const OptionsIterator = struct {
    ...
    /// Return remaining arguments, if any.
    pub fn args_iter(self: *OptionsIterator) !ArgIterator {
        if (self.args()) |list| {
            return ArgIterator.init(list);
        } else {
            return error.NoArgs;
        }
    }
...
pub const ArgIterator = struct {
    ...
    pub fn init(args: [][*:0]const u8) ArgIterator {
        return ArgIterator{
            .args = args,
            .index = 0,
            .count = args.len,
